...
protected_attr_val_2 = "non_white"
protected_attr_proxy = ['priors_count_cat']

cols_ohe =  ['sex', 'age_cat', 'c_charge_degree', 'race_cat',
            'juv_fel_count_cat', 'juv_misd_count_cat', 'juv_other_count_cat',
            'priors_count_cat', 'days_b_screening_arrest_cat', 'c_days_from_compas_cat']

original_data_size = 6142
...
dag0.add_nodes_from(['sex', 'age_cat', 'c_charge_degree', 'two_year_recid', 'race_cat',
                    'juv_fel_count_cat', 'juv_misd_count_cat', 'juv_other_count_cat',
                    'priors_count_cat', 'days_b_screening_arrest_cat', 'c_days_from_compas_cat'])


## Add edges
dag0.add_edge('race_cat', 'age_cat')
dag0.add_edge('age_cat', 'priors_count_cat')
dag0.add_edge('age_cat', 'juv_other_count_cat')
dag0.add_edge('priors_count_cat', 'two_year_recid')
dag0.add_edge('priors_count_cat', 'c_days_from_compas_cat')
dag0.add_edge('priors_count_cat', 'juv_misd_count_cat')
dag0.add_edge('priors_count_cat', 'juv_fel_count_cat')
dag0.add_edge('priors_count_cat', 'c_charge_degree')
dag0.add_edge('sex', 'priors_count_cat')
dag0.add_edge('c_days_from_compas_cat', 'days_b_screening_arrest_cat')


compas_bn_0 = BayesianNetwork(ebunch=dag0.edges())
compas_bn_0.fit(
    data=df_compas,
...
## forward sample
inference = BayesianModelSampling(compas_bn_0)
compas_bn_1_sample_0 = inference.forward_sample(size=6172, include_latents=False, seed=seed_bn)

## Export the data
compas_bn_1_sample_0.to_csv(f'{folder_name}/{log_folder_name}/compas_initial_graph.csv', index=False)
...
dag1.add_nodes_from(['sex', 'age_cat', 'c_charge_degree', 'two_year_recid', 'race_cat',
                    'juv_fel_count_cat', 'juv_misd_count_cat', 'juv_other_count_cat',
                    'priors_count_cat', 'days_b_screening_arrest_cat', 'c_days_from_compas_cat'])
...
## forward sample
inference = BayesianModelSampling(compas_bn_1)
compas_bn_1_sample_1 = inference.forward_sample(size=6172, include_latents=False, seed=seed_bn)

## Export the data
compas_bn_1_sample_1.to_csv(f'{folder_name}/{log_folder_name}/compas_without_fairness.csv', index=False)
...
df_compas_1['two_year_recid'] = df_compas_1['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
df00_1, X00, y00 = preprocess_data(df_compas_1, cols_ohe, outcome_name)
...
##---------------- 
df01 = df_compas_1.drop(columns='race_cat')    
cols_ohe_1 =  ['sex', 'age_cat', 'c_charge_degree', 'juv_fel_count_cat', 'juv_misd_count_cat', 'juv_other_count_cat',
            'priors_count_cat', 'days_b_screening_arrest_cat', 'c_days_from_compas_cat']
...
##---------------- 
df02 = df_compas_1.drop(columns=['race_cat', 'priors_count_cat']) 
cols_ohe_2 =  ['sex', 'age_cat', 'c_charge_degree', 'juv_fel_count_cat', 'juv_misd_count_cat', 'juv_other_count_cat' , 'days_b_screening_arrest_cat', 'c_days_from_compas_cat']
...
##---------------- 
df0 = pd.read_csv(f'{folder_name}/{log_folder_name}/compas_initial_graph.csv')
## Swap the label
df0['two_year_recid'] = df0['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
...
##---------------- 
df1 = pd.read_csv(f'{folder_name}/{log_folder_name}/compas_without_fairness.csv')
## Swap the label
df1['two_year_recid'] = df1['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
...
for i, lambda_val in enumerate(lambda_values):
    ...
    ## Swap the label
    df31['two_year_recid'] = df31['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
    ...
    ## Swap the label
    df32['two_year_recid'] = df32['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
    ...
    ## Swap the label
    df33['two_year_recid'] = df33['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
    ...
    ## Swap the label
    df34['two_year_recid'] = df34['two_year_recid'].apply(lambda x: 1 if x == 0 else 0)
...
for i, df_name in enumerate(df_model_names):
    ...
    sf_col_name = 'race_cat'
    benchmark_col = "('race_cat_white',)"
    protected_col = "('race_cat_non_white',)"

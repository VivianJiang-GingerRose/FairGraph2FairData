...
def perform_cv(df_in, model_type, label_name, protected_col,  random_state=None, n_splits=5
    ...
    for train_idx, test_idx in cv.split(X, y):
        ...
        if bias_mit_method == 'Reweighing':
            ## Apply reweighing
            rw_tr = Reweighing(unprivileged_groups=unprivileged_group, privileged_groups=privileged_group)
            rw_tr.fit(df_train_1)
            df_train_trf = rw_tr.transform(df_train_1)

            rw_ts = Reweighing(unprivileged_groups=unprivileged_group, privileged_groups=privileged_group)
            rw_ts.fit(df_test_1)
            df_test_trf = rw_ts.transform(df_test_1)

            ##-----------------------------------
            # 1. Calculate model performance metrics
            ##-----------------------------------
            if model_type == "logistic_regression":
                model, y_pred, precision, recall, f1, roc_auc, accuracy_balanced = build_logistic_regression_model(df_train_trf.features, df_test_trf.features, df_train_trf.labels.ravel(), df_test_trf.labels.ravel(), bias_mit_method=bias_mit_method, sample_weight=df_train_trf.instance_weights)
            elif model_type == "random_forest":
                model, y_pred, precision, recall, f1, roc_auc, accuracy_balanced = build_random_forest_model(df_train_trf.features, df_test_trf.features, df_train_trf.labels.ravel(), df_test_trf.labels.ravel(), bias_mit_method=bias_mit_method, sample_weight=df_train_trf.instance_weights)
            elif model_type == "xgboost":
                model, y_pred, precision, recall, f1, roc_auc, accuracy_balanced = build_xgboost_model(df_train_trf.features, df_test_trf.features, df_train_trf.labels.ravel(), df_test_trf.labels.ravel(), bias_mit_method=bias_mit_method, sample_weight=df_train_trf.instance_weights)
            else:
                raise ValueError("Invalid model type. Choose 'logistic_regression', 'random_forest', or 'xgboost'.")
            ...
            DPD, DPR, EO = calc_fairness_metrics(y_test, y_pred, X_test[[protected_col]])
...
def run_samples(dataset_name, bias_mitigation_method, df_in, target_name, protected_col, random_state=None
    ...
    m_results = pd.DataFrame(columns=['dataset_name','bias_mitigation_method','model_type', 'precision', 'recall','f1', 'AUROC', 'accuracy','DPD', 'DPR','EO'])
    ...
    m_results_logreg = pd.DataFrame([{'dataset_name':dataset_name, 
                                      'bias_mitigation_method': "Pre-processing: Reweighing",

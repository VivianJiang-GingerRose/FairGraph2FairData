...
def build_logistic_regression_model(X_train,  X_test, y_train, y_test, bias_mit_method=None, sample_weight=None):
    ...
    if bias_mit_method == 'LearningFairRepresentation':
        ## Apply reweighing
...
def perform_cv(df_in, model_type, label_name, protected_col,  random_state=None, n_splits=5
    ...
    for train_idx, test_idx in cv.split(X, y):
        ...
        if bias_mit_method == 'LearningFairRepresentation':
            ## Apply DIR
            TR = LFR(unprivileged_groups=unprivileged_group, privileged_groups=privileged_group
                     , k=10, Ax=0.1, Ay=1.0, Az=2.0, verbose=1)
            # TR = TR.fit(df_train_1, maxiter=5000, maxfun=5000)
            TR = TR.fit(df_train_1, maxiter=50, maxfun=50)

            df_train_trf = TR.transform(df_train_1)
            df_test_trf = TR.transform(df_test_1)

            ## Check if there are two classes in df_train_trf.label and df_test_trf.label
            ## If there are not two classes, then calcualte the threshold that would give two classes.
            ## If we do not do this, there will be only one class in our training and testing data.
            unique_classes_train = np.unique(df_train_trf.labels)
            # unique_classes_test = np.unique(df_test_trf.labels)

            if len(unique_classes_train) < 2:
                # Calculate the threshold that would give two classes
                threshold_tr = (np.min(df_train_trf.scores) + np.max(df_train_trf.scores)) / 2
                df_train_trf = TR.transform(df_train_1, threshold=threshold_tr)

            X_train = df_train_trf.features
            X_test = df_test_trf.features 
            y_train = np.squeeze(df_train_trf.labels)
            ## y_test is the original label, not the transformed label
...
def run_samples(dataset_name, bias_mitigation_method, df_in, target_name, protected_col, random_state=None
    ...
    m_results_logreg = pd.DataFrame([{'dataset_name':dataset_name, 
                                      'bias_mitigation_method': "Pre-processing: Learning Fair Representation",

...
from pgmpyVJ.factors.base import BaseFactor
...
class FactorSet(object):
    ...
    def copy(self):
        ...
        >>> factor_set = FactorSet(phi1, phi2)
        >>> factor_set
        <pgmpyVJ.factors.FactorSet.FactorSet at 0x7fa68f390320>
        >>> factor_set_copy = factor_set.copy()
        >>> factor_set_copy
        <pgmpyVJ.factors.FactorSet.FactorSet at 0x7f91a0031160>
        """
...
def factorset_product(*factorsets_list):
    ...
    >>> from pgmpyVJ.factors import FactorSet
    >>> from pgmpyVJ.factors.discrete import DiscreteFactor
    >>> from pgmpyVJ.factors import factorset_product
    >>> phi1 = DiscreteFactor(['x1', 'x2', 'x3'], [2, 3, 2], range(12))
    >>> phi2 = DiscreteFactor(['x3', 'x4', 'x1'], [2, 2, 2], range(8))
    >>> factor_set1 = FactorSet(phi1, phi2)
    >>> phi3 = DiscreteFactor(['x5', 'x6', 'x7'], [2, 2, 2], range(8))
...
def factorset_divide(factorset1, factorset2):
    ...
    >>> from pgmpyVJ.factors import FactorSet
    >>> from pgmpyVJ.factors.discrete import DiscreteFactor
    >>> from pgmpyVJ.factors import factorset_divide
    >>> phi1 = DiscreteFactor(['x1', 'x2', 'x3'], [2, 3, 2], range(12))
    >>> phi2 = DiscreteFactor(['x3', 'x4', 'x1'], [2, 2, 2], range(8))
    >>> factor_set1 = FactorSet(phi1, phi2)
    >>> phi3 = DiscreteFactor(['x5', 'x6', 'x7'], [2, 2, 2], range(8))

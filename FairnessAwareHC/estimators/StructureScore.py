...
class StructureScore(BaseEstimator):
    def __init__(self, data, **kwargs):
        """
        Abstract base class for structure scoring classes in FairnessAwareHC. Use any of the derived classes
    ...
    def score(self, model):
        ...
        >>> from FairnessAwareHC.estimators import K2Score
...
class BicScore(StructureScore):
    ...
    def local_score(self, variable, parents):
        ...
        # score -= 0.5 * log(sample_size) * (num_parents_states * (var_cardinality - 1))/r
        ## Where r = 1 when the variable is target variable and r > 1(user define) if it is non-target

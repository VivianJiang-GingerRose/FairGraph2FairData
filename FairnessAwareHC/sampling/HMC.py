...
from FairnessAwareHC.sampling import (
    BaseGradLogPDF,
...
class HamiltonianMC(object):
    ...
    Parameters
    ----------
    model: An instance FairnessAwareHC.models
    ...
    simulate_dynamics: A subclass of FairnessAwareHC.inference.continuous.BaseSimulateHamiltonianDynamics
        A class to propose future values of momentum and position in time by simulating
        Hamiltonian Dynamics

    Example
    -------
    >>> from FairnessAwareHC.sampling import HamiltonianMC as HMC, LeapFrog, GradLogPDFGaussian
    >>> from FairnessAwareHC.factors.continuous import GaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([-3, 4])
    ...
    def __init__(self, model, grad_log_pdf, simulate_dynamics=LeapFrog):
        if not issubclass(grad_log_pdf, BaseGradLogPDF):
            raise TypeError(
                ...
                + "FairnessAwareHC.inference.base_continuous.BaseGradLogPDF"
            )

        if not issubclass(simulate_dynamics, BaseSimulateHamiltonianDynamics):
            raise TypeError(
                "split_time must be an instance of "
                + "FairnessAwareHC.inference.base_continuous.BaseSimulateHamiltonianDynamics"
            )
    ...
    def sample(
        ...
        Examples
        --------
        >>> from FairnessAwareHC.sampling import HamiltonianMC as HMC, GradLogPDFGaussian, ModifiedEuler
        >>> from FairnessAwareHC.factors.continuous import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, -1])
    ...
    def generate_sample(
        ...
        Examples
        --------
        >>> from FairnessAwareHC.sampling import HamiltonianMC as HMC, GradLogPDFGaussian as GLPG
        >>> from FairnessAwareHC.factors import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([4, -1])
...
class HamiltonianMCDA(HamiltonianMC):
    ...
    grad_log_pdf: A subclass of FairnessAwareHC.inference.continuous.GradientLogPDF
    ...
    Example
    -------
    >>> from FairnessAwareHC.sampling import HamiltonianMCDA as HMCda, LeapFrog, GradLogPDFGaussian as GLPG
    >>> from FairnessAwareHC.factors.continuous import GaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([1, 2, 3])
    ...
    def sample(
        ...
        Examples
        ---------
        >>> from FairnessAwareHC.sampling import HamiltonianMCDA as HMCda, GradLogPDFGaussian as GLPG, LeapFrog
        >>> from FairnessAwareHC.factors.continuous import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, 1])
